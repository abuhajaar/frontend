...
import { animatePomodoroExit } from '@/component/PomodoroTransition';
...
export default function PomodoroPage() {
    ...
    useEffect(() => {
        const storedPos = sessionStorage.getItem('pomodoroButtonPos');
        const pos = storedPos ? JSON.parse(storedPos) : { x: 50, y: 50 };
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-[200] bg-black text-white selection:bg-white selection:text-black flex flex-col">
            ...
            <div className="absolute top-0 left-0 w-full p-8 flex justify-between items-center z-50">
                <button
                    onClick={() => {
                        // Get stored button position
                        const storedPos = sessionStorage.getItem('pomodoroButtonPos');
                        const targetPos = storedPos ? JSON.parse(storedPos) : { x: 50, y: 50 };
                        
                        // Animate page clip-path to shrink into circle
                        gsap.to(containerRef.current, {
                            clipPath: `circle(0% at ${targetPos.x}% ${targetPos.y}%)`,
                            duration: 0.7,
                            ease: 'power2.inOut',
                            onComplete: () => {
